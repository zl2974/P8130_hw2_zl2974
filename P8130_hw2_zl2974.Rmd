...
1. 
$$\begin{aligned}
sensitivity = \frac{\boldsymbol{P}(positive \mid carcinoma)}{\boldsymbol{P}(carcinoma)}\\
=\frac{79/113}{79/113+10/217}\\=
...
3. 
    a. $$\begin{aligned}
    \boldsymbol{P}(prostatic\ cancer\mid Positive\cap Palpable\ Prostatic\ Nodule)\\=\frac{P(Positive\cap Palpable\ Prostatic\ Nodule\ \cap prostatic\ cancer)}{P(Positive\cap Palpable\ Prostatic\ Nodule)}\\
    ={P(Positive\cap Palpable\ Prostatic\ Nodule\ \cap prostatic\ cancer)} \div \\(P(Positive\cap Palpable\ Prostatic\ Nodule\mid prostatic\ cancer) \times P(prostatic\ cancer)\\+ P(Positive\cap Palpable\ Prostatic\ Nodule\mid prostatic\ cancer^c) \times P(prostatic\ cancer^c))\\=\frac{80\% \times 50\%}{80\% \times 50\% +5\% \times 50\%}\\=`r .8*.5/(.8*.5+.05*.5)`
    \end{aligned}$$
\ This is the positive predictive value of a test.

    a. 
    $$\begin{aligned}
    \boldsymbol{P}(prostatic\ cancer'\mid Positive\cap Palpable\ Prostatic\ Nodule)\\=\frac{P(Positive\cap Palpable\ Prostatic\ Nodule\ \cap prostatic\ cancer')}{P(Positive\cap Palpable\ Prostatic\ Nodule)}\\={P(Positive\cap Palpable\ Prostatic\ Nodule\ \cap prostatic\ cancer')} \div \\(P(Positive\cap Palpable\ Prostatic\ Nodule\mid prostatic\ cancer') \times P(prostatic\ cancer')\\+ P(Positive\cap Palpable\ Prostatic\ Nodule\mid prostatic\ cancer'^c) \times P(prostatic\ cancer'^c))\\=\frac{80\% \times 10\%}{80\% \times 10\% +5\% \times 90\%}\\=`r .8*.1/(.8*.1+.05*.9)`
    \end{aligned}$$
...
1. the sample size exceed 30, it can be approximate by Normal distribution of $X \sim N(np,np(1-p))$ by CLT
    ...
    1. 
    $P(\mathcal{X}=17)\ \approx P_N(17-1/2 <\mathcal{X}<17+1/2)\\
    = `r pnorm(17.5,17.25,sqrt(50*.345*(1-.345)))-pnorm(16.5,17.25,sqrt(50*.345*(1-.345)))`$
    
```{r approximation,echo=F,message=FALSE}
library(tidyverse)
tibble(
    normal = rnorm(500,17.25,sqrt(50*.345*(1-.345))),
    binomial = rbinom(500,50,.345)
  ) %>% 
  pivot_longer(
    normal:binomial,
    names_to = 'dis',
  ...
  viridis::scale_fill_viridis(name = "Distribution",
                              discrete = T)+
  labs(
    title = "Distribution of 500 samples from Simulation"
  )
```
...
And given this is the number of rare disease in large population, a Poisson distribution can be used to approximate the distribution of population.<br/>
...
$$
\begin{aligned}
P_X(x=30)= e^{-\lambda}\lambda^k/k!=`r ppois(30,5*8.3)`
\end{aligned}
$$

1. There're `r .14*8.3` million asians, `r .428*8.3` million non-Hispanic white, `r .243*8.3` million black and `r (1-(.14+.428+.243))*8.3` million other unspecified ethnic/racial groups. The $\lambda$ for these subpopulation within their group are `r .14*8.3*.31`, `r .428*8.3*6.02`, `r .243*8.3*.39` and `r (1-.811)*8.3*5` respectively. The sum of $\lambda'$ is `r .14*8.3*.31+ .428*8.3*6.02+ .243*8.3*.39+ (1-.811)*8.3*5` for the updated NYC population by joint distribution of poisson distribution.
$$
\begin{aligned}
P_X(x=30)= e^{-\lambda'}\lambda'^k/k!=`r ppois(30, .14*8.3*.31+ .428*8.3*6.02+ .243*8.3*.39+ (1-.811)*8.3*5)`\\
\blacksquare
\end{aligned}
$$
...
$$
\begin{aligned}
P_{\bar{X}}(\bar{x}<98) = \phi(k < \frac{98-99.99}{.73/\sqrt{40}}) \\
= `r pnorm(98,99.99,.73/sqrt(40))`
\end{aligned}
$$
...
$$
\begin{aligned}
P_{\bar{X}}(\bar{x}>100.5) = 1 - \phi(k < \frac{100.5-99.99}{.73/\sqrt{40}}) \\
= `r 1- pnorm(100.5,99.99,.73/sqrt(40))`
\end{aligned}
$$

1. `r qnorm(.9,99.99,.73/sqrt(40))`

1. left cutoff is $25^{th}$ percentile of the sampling distribution, which is `r qnorm(.25,99,.73/sqrt(40))`. And the right is the $75^{th}$ `r qnorm(.75,99,.73/sqrt(40))`.<br/>


```{r cutoff, echo = F, message=F}
plt_df = tibble(
  value = rnorm(10000,99,.73/sqrt(40))
) %>% 
  mutate(cutoff = case_when(
    value < qnorm(.25,99,.73/sqrt(40)) ~ "outside mid-50%",
    value > qnorm(.75,99,.73/sqrt(40)) ~ "outside mid-50%",
    value != "outside mid-50%" ~ "mid-50%"
  ),
  y = dnorm(value,99,.73/sqrt(40))
  )
ggplot(data = plt_df,
       aes(x=value,y=y)
       )+
  ...
  geom_area(data = filter(plt_df,cutoff == "mid-50%",
                          between(value,
                                  qnorm(.25,99,.73/sqrt(40)),
                                  qnorm(.75,99,.73/sqrt(40)))), 
            aes(x = value, y = y,fill = cutoff)
            )+
  scale_x_continuous(
    breaks = c(qnorm(.25,99,.73/sqrt(40)),qnorm(.75,99,.73/sqrt(40))),
  )+
  labs(x = "", y = "")+
